                        ...
                        <ul class="sidebar-left-icons nav nav-pills" id="add-product-pills-tab" role="tablist">
                          <li class="nav-item"> <a class="nav-link active" id="detail-product-tab" data-bs-toggle="pill" href="#detail-product" role="tab" aria-controls="detail-product" aria-selected="false">
                              ...
                              <div class="product-tab-content">
                                <h6>Edit Career Details</h6>
                                <p>Edit Career name & details</p>
                              </div></a></li>
...
@php
  $isEdit = isset($career);
  // Parse responsibilities - Laravel's array cast will auto-decode JSON
  $responsibilities = [];
...
<form method="POST" action="{{ $isEdit ? route('admin.career.update', $career->id) : route('admin.career.store') }}">
  ...
  <div class="col-md-6">
    ...
    <input class="form-control" name="location" type="text" placeholder="e.g., Karachi, Pakistan" value="{{ old('location', $career->location ?? '') }}">
  </div>

  <div class="col-md-12">
    <label class="form-label">Job Summary</label>
    <textarea class="form-control" name="summary" rows="4" placeholder="Enter job summary..." required>{{ old('summary', $career->summary ?? '') }}</textarea>
  </div>
  ...
  <div class="col-md-6">
    ...
    <input class="form-control" name="education" type="text" placeholder="e.g., Bachelor's Degree" value="{{ old('education', $career->education ?? '') }}">
  </div>

  <div class="col-md-6">
    <label class="form-label">Experience</label>
    <input class="form-control" name="experience" type="text" placeholder="e.g., 3+ Years" value="{{ old('experience', $career->experience ?? '') }}">
  </div>

  <div class="col-md-6">
    <label class="form-label">Salary Range</label>
    <input class="form-control" name="salary_range" type="text" placeholder="e.g., 70k - 100k" value="{{ old('salary_range', $career->salary_range ?? '') }}">
  </div>

  <div class="col-md-6">
    <label class="form-label">Visibility</label>
    <select class="form-control" name="visibility">
      <option value="1" {{ old('visibility', $career->visibility ?? 1) == 1 ? 'selected' : '' }}>Visible</option>
      <option value="0" {{ old('visibility', $career->visibility ?? 1) == 0 ? 'selected' : '' }}>Hidden</option>
    </select>
  </div>
